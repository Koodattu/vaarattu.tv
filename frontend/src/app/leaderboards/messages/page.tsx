...
function MessagesLeaderboardContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await apiClient.getTopUsers("messages", timeRange, page, pageSize);
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
            <span>💬</span> Most Messages
          </h1>
          <p className="text-gray-400">Viewers who have sent the most chat messages</p>
        </div>
                  ...
                  return (
                    <div key={user.id} className="flex items-center gap-4 p-4 hover:bg-gray-700/50 transition-colors">
                      ...
                      <div className="text-right">
                        <div className="text-white font-bold text-xl">{formatNumber(user.totalMessages)}</div>
...
export default function MessagesLeaderboardPage() {
  return (
